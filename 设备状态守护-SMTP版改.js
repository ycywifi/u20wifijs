...
(async () => {
    ...
    const state = {
        ...
        config: {
            ...
            smtpHost: localStorage.getItem(LS_KEY + 'smtpHost') || 'smtp.qq.com',
            smtpPort: parseInt(localStorage.getItem(LS_KEY + 'smtpPort')) || 465,
            smtpTls: localStorage.getItem(LS_KEY + 'smtpTls') || 'ssl',
            smtpUser: localStorage.getItem(LS_KEY + 'smtpUser') || '',
            smtpPass: localStorage.getItem(LS_KEY + 'smtpPass') || '',
            mailFrom: localStorage.getItem(LS_KEY + 'mailFrom') || '',
            mailTo: localStorage.getItem(LS_KEY + 'mailTo') || '',
        },
    };
...
trap 'kill $(jobs -p) 2>/dev/null; log "设备状态守护已退出"; exit 0' INT TERM
 ...
 SMTP_HOST=${sq(state.config.smtpHost)}
 SMTP_PORT=${sq(state.config.smtpPort)}
 SMTP_TLS=${sq(state.config.smtpTls)}
 SMTP_USER=${sq(state.config.smtpUser)}
 SMTP_PASS=${sq(state.config.smtpPass)}
 MAIL_FROM=${sq(state.config.mailFrom)}
 MAIL_TO=${sq(state.config.mailTo)}

CURL_BIN="/data/data/com.minikano.f50_sms/files/curl"
[ ! -x "$CURL_BIN" ] && CURL_BIN="curl"
CURL_DIR=$(dirname "$CURL_BIN" 2>/dev/null)
CURL_VER=$("$CURL_BIN" -V 2>/dev/null)
log "[工具] curl 支持协议: $(echo "$CURL_VER" | sed -n 's/^Protocols: *//p')"
OPENSSL_BIN=$(command -v openssl 2>/dev/null)
[ -z "$OPENSSL_BIN" ] && [ -x "$CURL_DIR/openssl" ] && OPENSSL_BIN="$CURL_DIR/openssl"
NC_BIN=$(command -v nc 2>/dev/null)
[ -z "$NC_BIN" ] && [ -x "$CURL_DIR/nc" ] && NC_BIN="$CURL_DIR/nc"
B64_BIN=$(command -v base64 2>/dev/null)
[ -z "$B64_BIN" ] && [ -x "$CURL_DIR/base64" ] && B64_BIN="$CURL_DIR/base64"
log "[工具] openssl: \${OPENSSL_BIN:-无} | nc: \${NC_BIN:-无} | base64: \${B64_BIN:-无}"
...
# 消息分段换行符（真实换行，POSIX sh 兼容）
NL="
"


# ─── SMTP 发送（多方式：openssl / curl / nc）───
smtp_curl_supported() {
    echo "$CURL_VER" | grep -qi 'smtp' || return 1
    if [ "$SMTP_TLS_MODE" != "none" ]; then
        echo "$CURL_VER" | grep -qiE 'smtps|SSL' || return 1
    fi
    return 0
}
...
b64() {
    if [ -n "$B64_BIN" ]; then
        printf '%s' "$1" | "$B64_BIN" | tr -d '\n'
    elif [ -x "$OPENSSL_BIN" ]; then
        printf '%s' "$1" | "$OPENSSL_BIN" base64 -A 2>/dev/null | tr -d '\n'
    else
        printf '%s' "$1"
    fi
}
...
smtp_send() {
    printf '%s%s' "$1" "$SMTP_EOL" >&3
}

smtp_read() {
    SMTP_TRIES=0
    while :; do
        line=$(tail -c +$((SMTP_OFF+1)) "$SMTP_RESP" 2>/dev/null | head -n 1)
        if [ -n "$line" ]; then
            SMTP_OFF=$(wc -c < "$SMTP_RESP")
            SMTP_LAST=$(echo "$line" | awk '{print substr($0,1,3)}')
            [ -n "$SMTP_LAST" ] && return 0
        fi
        SMTP_TRIES=$((SMTP_TRIES+1))
        [ "$SMTP_TRIES" -gt 100 ] && return 1
        sleep 0.2
    done
}
...
smtp_dialog() {
    smtp_send "EHLO flowguard" || return 1
    smtp_expect 250 || return 1
    smtp_send "AUTH LOGIN" || return 1
    smtp_expect 334 || return 1
    smtp_send "$(b64 "$SMTP_USER")" || return 1
    smtp_expect 334 || return 1
    smtp_send "$(b64 "$SMTP_PASS")" || return 1
    smtp_expect 235 || return 1
    smtp_send "MAIL FROM:<$MAIL_FROM>" || return 1
    smtp_expect 250 || return 1
    smtp_send "RCPT TO:<$MAIL_TO>" || return 1
    smtp_expect 250 || return 1
    smtp_send "DATA" || return 1
    smtp_expect 354 || return 1
    cat "$1" >&3
    printf '.\r\n' >&3
    smtp_expect 250 || return 1
    smtp_send "QUIT" || return 1
    return 0
}

smtp_send_openssl() {
    SMTP_EOL="\r\n"
    SMTP_FIFO="$DATA_DIR/mail_fifo.$$"
    SMTP_RESP="$DATA_DIR/mail_resp.$$"
    rm -f "$SMTP_FIFO" "$SMTP_RESP"
    mkfifo "$SMTP_FIFO" 2>/dev/null || return 1
    if [ "$SMTP_TLS_MODE" = "starttls" ]; then TLS_EXTRA="-starttls smtp"; else TLS_EXTRA=""; fi
    ( "$OPENSSL_BIN" s_client -quiet $TLS_EXTRA -connect "$SMTP_HOST:$SMTP_PORT" < "$SMTP_FIFO" > "$SMTP_RESP" 2>"$DATA_DIR/mail_ssl_err.$$" ) &
    SMTP_PID=$!
    exec 3>"$SMTP_FIFO"
    SMTP_OFF=0
    if [ "$SMTP_TLS_MODE" != "starttls" ]; then
        smtp_expect 220 || {
            ...
            log "[通知] openssl 连接失败: $(tail -c +1 "$SMTP_RESP" 2>/dev/null | head -n 2 | tr '\n' ' ')"
            return 1
        }
    fi
    if smtp_dialog "$1"; then
        exec 3>&-
        kill "$SMTP_PID" 2>/dev/null
        return 0
    fi
    log "[通知] openssl 发送失败: $(tail -n 2 "$SMTP_RESP" 2>/dev/null | tr '\n' ' ')$(tr '\n' ' ' < "$DATA_DIR/mail_ssl_err.$$" 2>/dev/null)"
    exec 3>&-
    kill "$SMTP_PID" 2>/dev/null
    return 1
}
...
smtp_send_nc() {
    [ -x "$NC_BIN" ] || return 1
    SMTP_EOL="\r\n"
    SMTP_FIFO="$DATA_DIR/mail_fifo_nc.$$"
    SMTP_RESP="$DATA_DIR/mail_resp_nc.$$"
    rm -f "$SMTP_FIFO" "$SMTP_RESP"
    mkfifo "$SMTP_FIFO" 2>/dev/null || return 1
    ( "$NC_BIN" "$SMTP_HOST" "$SMTP_PORT" < "$SMTP_FIFO" > "$SMTP_RESP" 2>&1 ) &
    SMTP_PID=$!
    exec 3>"$SMTP_FIFO"
    SMTP_OFF=0
    smtp_expect 220 || { exec 3>&-; kill "$SMTP_PID" 2>/dev/null; return 1; }
    if smtp_dialog "$1"; then
        exec 3>&-
        kill "$SMTP_PID" 2>/dev/null
        return 0
...
smtp_send_curl() {
    if [ "$SMTP_TLS_MODE" = "ssl" ]; then
        SMTP_URL="smtps://$SMTP_HOST:$SMTP_PORT"
        TLS_ARGS=""
    elif [ "$SMTP_TLS_MODE" = "starttls" ]; then
    ...
    notify_out=$("$CURL_BIN" -sS --url "$SMTP_URL" $TLS_ARGS --user "$SMTP_USER:$SMTP_PASS" --mail-from "$MAIL_FROM" --mail-rcpt "$MAIL_TO" --upload-file "$1" 2>&1)
    notify_ret=$?
    if [ "$notify_ret" -eq 0 ]; then
        return 0
    fi
    log "[通知] curl 方式发送失败(代码:$notify_ret) - $notify_out"
    return 1
}

notify() {
    [ -z "$SMTP_HOST" ] || [ -z "$SMTP_USER" ] || [ -z "$SMTP_PASS" ] || [ -z "$MAIL_FROM" ] || [ -z "$MAIL_TO" ] && { log "[通知] SMTP 配置不完整，跳过发送"; return 1; }

    log "[通知] 正在通过 SMTP 发送报警邮件..."
    msg=$(printf '%s' "$1")

    mkdir -p "$DATA_DIR"
    {
        printf 'From: =?UTF-8?B?%s?= <%s>\r\n' "$(b64 "设备状态守护")" "$MAIL_FROM"
        printf 'To: <%s>\r\n' "$MAIL_TO"
        printf 'Subject: =?UTF-8?B?%s?=\r\n' "$(b64 "设备状态提醒")"
        printf 'Content-Type: text/plain; charset=UTF-8\r\n'
        printf '\r\n'
        printf '%s' "$msg" | awk '{printf "%s\r\n", $0}'
    } > "$DATA_DIR/mail_body.txt"

    SMTP_TLS_MODE=$SMTP_TLS
    [ -z "$SMTP_TLS_MODE" ] && SMTP_TLS_MODE=ssl

    if [ "$SMTP_TLS_MODE" = "ssl" ] || [ "$SMTP_TLS_MODE" = "starttls" ]; then
        if [ -x "$OPENSSL_BIN" ]; then
            if smtp_send_openssl "$DATA_DIR/mail_body.txt"; then
                log "[通知] 邮件发送成功(openssl)"
                return 0
            fi
        else
            log "[通知] 未找到 openssl，跳过 openssl 方式"
        fi
        if smtp_curl_supported; then
            if smtp_send_curl "$DATA_DIR/mail_body.txt"; then
                log "[通知] 邮件发送成功(curl)"
                return 0
            fi
        else
            log "[通知] 当前 curl 不支持 SMTP/SSL 协议"
        fi
    else
        if smtp_curl_supported; then
            if smtp_send_curl "$DATA_DIR/mail_body.txt"; then
            ...
            log "[通知] 当前 curl 不支持 SMTP 协议"
        fi
        if [ -x "$NC_BIN" ]; then
            if smtp_send_nc "$DATA_DIR/mail_body.txt"; then
                log "[通知] 邮件发送成功(nc)"
                return 0
            fi
        else
            log "[通知] 未找到 nc，无法使用明文 SMTP"
        fi
    fi
    log "[通知] 所有发送方式均失败"
    return 1
...
while true; do
    ...
    if [ -n "\$MSG_FLOW_MONTH" ]; then
        [ -n "\$FINAL_MSG" ] && FINAL_MSG="\$FINAL_MSG\$NL\$NL"
...
for iface in $(ls /sys/class/net/ 2>/dev/null); do
    ...
    const saveToLocalStorage = () => {
        ...
        localStorage.setItem(LS_KEY + 'smtpHost', String(state.config.smtpHost || ''));
        localStorage.setItem(LS_KEY + 'smtpPort', String(state.config.smtpPort || 465));
        localStorage.setItem(LS_KEY + 'smtpTls', String(state.config.smtpTls || 'ssl'));
        localStorage.setItem(LS_KEY + 'smtpUser', String(state.config.smtpUser || ''));
        localStorage.setItem(LS_KEY + 'smtpPass', String(state.config.smtpPass || ''));
        localStorage.setItem(LS_KEY + 'mailFrom', String(state.config.mailFrom || ''));
        localStorage.setItem(LS_KEY + 'mailTo', String(state.config.mailTo || ''));
    };
...
`);
        ...
        if (configPart.trim()) {
            try {
                ...
                if (parsed.smtpHost !== undefined) state.config.smtpHost = parsed.smtpHost;
                if (parsed.smtpPort !== undefined) state.config.smtpPort = parsed.smtpPort;
                if (parsed.smtpTls !== undefined) state.config.smtpTls = parsed.smtpTls;
                if (parsed.smtpUser !== undefined) state.config.smtpUser = parsed.smtpUser;
                if (parsed.smtpPass !== undefined) state.config.smtpPass = parsed.smtpPass;
                if (parsed.mailFrom !== undefined) state.config.mailFrom = parsed.mailFrom;
                if (parsed.mailTo !== undefined) state.config.mailTo = parsed.mailTo;
                saveToLocalStorage();
    ...
    const install = async () => {
        try {
            ...
            const cfgObj = {
                ...
                smtpTls: state.config.smtpTls,
                smtpUser: state.config.smtpUser,
                smtpPass: state.config.smtpPass,
                mailFrom: state.config.mailFrom,
                mailTo: state.config.mailTo,
            };
    ...
    const ensureStyle = () => {
      ...
      #${MODAL} .fg-select{padding:5px 8px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:rgba(0,0,0,.18);color:inherit;font-size:.7rem;outline:none;box-sizing:border-box;}
      #${MODAL} .fg-select:focus{border-color:rgba(77,219,138,.5);}
    ...
    const render = () => {
      ...
      <div class="fg-wrap">
        <div class="fg-card">
          ...
          <div class="fg-help">配置通知触发阈值，合并推送至邮箱</div>
          ...
          <div class="fg-divider"></div>
          <div class="fg-form-row">
            <span class="fg-label">SMTP</span>
            <input class="fg-input" style="flex:1;width:auto;text-align:left;" data-field="smtpHost" type="text" value="${esc(cfg.smtpHost)}" placeholder="服务器，如 smtp.qq.com">
          </div>
          <div class="fg-form-row">
            <span class="fg-label">端口</span>
            <input class="fg-input" data-field="smtpPort" type="number" step="1" min="1" value="${esc(cfg.smtpPort)}">
            <select class="fg-select" data-field="smtpTls">
              <option value="ssl" ${cfg.smtpTls === 'ssl' ? 'selected' : ''}>SSL(465)</option>
              <option value="starttls" ${cfg.smtpTls === 'starttls' ? 'selected' : ''}>STARTTLS(587)</option>
              <option value="none" ${cfg.smtpTls === 'none' ? 'selected' : ''}>无加密(25)</option>
            </select>
            <span class="fg-unit">加密方式</span>
          </div>
          <div class="fg-form-row">
            <span class="fg-label">账号</span>
            <input class="fg-input" style="flex:1;width:auto;text-align:left;" data-field="smtpUser" type="text" value="${esc(cfg.smtpUser)}" placeholder="登录账号（QQ/163邮箱地址）">
          </div>
          <div class="fg-form-row">
            <span class="fg-label">授权码</span>
            <input class="fg-input" style="flex:1;width:auto;text-align:left;" data-field="smtpPass" type="password" value="${esc(cfg.smtpPass)}" placeholder="邮箱授权码或密码">
          </div>
          <div class="fg-form-row">
            <span class="fg-label">发件人</span>
            <input class="fg-input" style="flex:1;width:auto;text-align:left;" data-field="mailFrom" type="text" value="${esc(cfg.mailFrom)}" placeholder="发件邮箱地址">
          </div>
          <div class="fg-form-row">
            <span class="fg-label">收件人</span>
            <input class="fg-input" style="flex:1;width:auto;text-align:left;" data-field="mailTo" type="text" value="${esc(cfg.mailTo)}" placeholder="接收提醒的邮箱地址">
          </div>
    ...
    const syncStateFromForm = () => {
        ...
        const smtpHostInput = root.querySelector('[data-field="smtpHost"]');
        const smtpPortInput = root.querySelector('[data-field="smtpPort"]');
        const smtpTlsInput = root.querySelector('[data-field="smtpTls"]');
        const smtpUserInput = root.querySelector('[data-field="smtpUser"]');
        const smtpPassInput = root.querySelector('[data-field="smtpPass"]');
        const mailFromInput = root.querySelector('[data-field="mailFrom"]');
        const mailToInput = root.querySelector('[data-field="mailTo"]');
        ...
        if (smtpHostInput) state.config.smtpHost = smtpHostInput.value.trim();
        if (smtpPortInput) {
            const num = parseInt(smtpPortInput.value, 10);
            if (!isNaN(num) && num > 0) state.config.smtpPort = num;
        }
        if (smtpTlsInput) state.config.smtpTls = smtpTlsInput.value;
        if (smtpUserInput) state.config.smtpUser = smtpUserInput.value.trim();
        if (smtpPassInput) state.config.smtpPass = smtpPassInput.value.trim();
        if (mailFromInput) state.config.mailFrom = mailFromInput.value.trim();
        if (mailToInput) state.config.mailTo = mailToInput.value.trim();
        saveToLocalStorage();
    };
    ...
    // ─── help ──────────────────────────────────────────────────────────────
    const HELP_TEXT = `<b>功能说明</b><br>后台每 60 秒检测一次数据，当“限额”或“温度”超标时，将自动通过 SMTP 发送邮件提醒（邮件标题为“设备状态提醒”，正文为合并后的报警消息）。<br><br><b>使用配置说明</b><br>在下方填写 SMTP 服务器、端口、加密方式、账号、授权码（QQ/163 邮箱需开启 SMTP 服务并生成授权码）、发件人与收件人电子邮箱。<br>任意限额填写 <b>0</b> 即代表关闭该项的检测。<br>修改参数后请点击【保存配置】，如果当前状态是运行中的，它将自动热重启使配置生效。<br><br><b>报警锁定规则</b><br>为避免刷屏，每项报警触发后会被锁定：<br>· 当日流量/温度：次日凌晨自动重置。<br>· 当月流量：次月 1 号自动重置。<br>若某项已触发报警，其对应的按钮会变红，您可以随时点击红色的【重置状态】手动清除锁定，使其能立刻再次报警。`;
